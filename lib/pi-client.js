...
const API_BASE = process.env.PI_API_BASE_URL || "https://api.testnet.minepi.com";
...
class PiClient {
  async createPayment(paymentData) {
    try {
      logger.info("Creating payment request", paymentData);
      const { data } = await api.post("/v2/payments", paymentData);
      return data;
    } catch (err) {
  ...
  async getPayment(paymentId) {
    try {
      ...
      const { data } = await api.get(`/v2/payments/${paymentId}`);
      return data;
    } catch (err) {
  ...
  async approvePayment(paymentId) {
    try {
      ...
      const { data } = await api.post(`/v2/payments/${paymentId}/approve`);
      return data;
    } catch (err) {
  ...
  async completePayment(paymentId, txid) {
    try {
      ...
      const { data } = await api.post(`/v2/payments/${paymentId}/complete`, { txid });
      return data;
    } catch (err) {
  ...
  async cancelPayment(paymentId) {
    try {
      ...
      const { data } = await api.post(`/v2/payments/${paymentId}/cancel`);
      return data;
    } catch (err) {
